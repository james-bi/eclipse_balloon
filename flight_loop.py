...
class TelemetryDispatcher:
    ...
    def send_logs(self) -> bool:
        ...
        try:
            ...
            print("=" * 80)
            print("LOG WEBHOOK REQUEST DETAILS (for server agent)")
            print("=" * 80)
            print(f"Endpoint URL: {url}")
            print(f"HTTP Method: POST")
            print(f"Request Headers: {json.dumps(headers)}")
            print(f"Request Payload (JSON):\n{json.dumps(payload, indent=2)}")
            print(f"Payload (raw string): {payload_json}")
            print("=" * 80)
            ...
            print(f"Response Body: {response.text}")
            response.raise_for_status()
            print(f"Log batch sent successfully ({len(logs)} records)")
            return True
...
class CameraManager:
    ...
    def send_webhook(self, filename: str, s3_url: str) -> bool:
        ...
        try:
            ...
            payload_json = json.dumps(payload)
            logger.info("=" * 80)
            logger.info("IMAGE WEBHOOK REQUEST DETAILS (for server agent)")
            logger.info("=" * 80)
            ...
            response.raise_for_status()
            logger.info(f"Webhook sent successfully for {filename}")
            return True
        except requests.exceptions.RequestException as e:
